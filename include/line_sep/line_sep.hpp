...
namespace line_sep {
    ...
    namespace gridding {
        void init_grid(const std::map<int32_t, line::LR_adj>& line_adjs, std::map<int32_t, std::shared_ptr<line_sep::line::GCol>>& grid, int ncells);
        void grid_lines(const std::map<int32_t, line::Line>& lines, const std::map<int32_t, line::LR_adj>& line_adjs, std::map<int32_t, std::shared_ptr<line::GCol>>& grid, double ymin_grid, double dy);
        std::tuple<int, double, double, double> get_number_cells(const std::map<int32_t, line::Line>& lines, double nomdst, double grid_tol);
    }
    namespace distance {
        void calc_line_dists(std::map<int32_t, line::Line>& lines, const std::map<int32_t, line::LR_adj>& line_adjs, const std::map<int32_t, std::shared_ptr<line::GCol>>& grid, double ymin_grid, double dy);
        void calc_dist(std::map<int32_t, line::Line>& lines, line::LR lr, const std::map<int32_t, std::shared_ptr<line::GCol>>& grid, int32_t lsymb_target, int32_t lsymb_adj, double ymin_grid, double dy);
    }  
}